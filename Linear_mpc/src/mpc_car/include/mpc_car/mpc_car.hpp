...
class MpcCar {
 private:
  ...
  void linearization(const double& phi,
    ...
    //       1 ,0;
    // gd_<< v * phi * sin(phi), -v * phi * cos(phi), -v * delta / (ll_ * cos(delta) * cos(delta)), 0;
    ...
    Bd_<< 0, 0,
          ...
          dt_ ,0;
    gd_<< v * phi * sin(phi) * dt_, -v * phi * cos(phi) * dt_, -v * delta * dt_ / (ll_ * cos(delta) * cos(delta)), 0;
    // std::cout << "Ad_:" << "\n" << Ad_ << std::endl;
    // std::cout << "Bd_:" << "\n" << Bd_ << std::endl;
    // std::cout << "gd_:" << "\n" << gd_ << std::endl;
 ...
 public:
  MpcCar(ros::NodeHandle& nh) : nh_(nh) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      // lx_.coeffRef( ...
      // ux_.coeffRef( ...
      Cx_.coeffRef(i, i * n + 3) = 1; // v at four position +3 !!!
      lx_.coeffRef(i, 0) = -0.1;
      ux_.coeffRef(i, 0) = v_max_;
    }
    std::cout << "constraints set sucess" << std::endl;
  ...
  int solveQP(const VectorX& x0_observe) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      last_phi = phi;
      std::cout << "start linear" << std::endl;
       ...
       *     X = BB * U + AA * x0 + gg
       * */
      // std::cout << "start set AA BB gg " << std::endl;
      if (i == 0) {
        BB.block(0, 0, n, m) = Bd_;
        AA.block(0, 0, n, n) = Ad_;
        gg.block(0, 0, n, 1) = gd_;
        BB_i.middleCols(0, m) = Bd_;
        AA_i.middleCols(0, n) = Ad_;
        gg_i.middleCols(0, 1) = gd_; 
      } else {

        for(int j = 0;j <i;j++){
          BB_i.block(0 , j * m, n, m) = Ad_ * BB_i.block(0, j * m, n, m);
          // BB_i.middleCols(j * m, m) = Ad_ * BB_i.middleCols(j * m, m);
        }
        // std::cout << "BB_i:" << "\n" << BB_i << std::endl;
        BB_i.middleCols(i * m, m) = Bd_;
        AA_i = Ad_ * AA_i;
        gg_i = Ad_ * gg_i + gd_;
        BB.block(n * i, 0, n, m * N_) = BB_i;
      ...
      std::cout << "set qx sucess" << std::endl;
