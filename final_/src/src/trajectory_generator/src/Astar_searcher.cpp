...
double AstarPathFinder::getHeu(GridNodePtr node1, GridNodePtr node2) {
  ...
  double h;
  Vector3i d = node1->index - node2->index;
  int dx = abs(d(0));
  int dy = abs(d(1));
  int dz = abs(d(2));
  ...
  h = sqrt(3) * dmin + sqrt(2) * (dmid - dmin) + (dmax - dmid);

  return h* (1 + tie_breaker);
}

void AstarPathFinder::AstarGraphSearch(Vector3d start_pt, Vector3d end_pt) {
  ros::Time time_1 = ros::Time::now();
  // can add a judge if endpt or startpt not in map return false
  ...
  while (!openSet.empty()) {
    ...
    if(currentPtr ->index == goalIdx){
      ...
      ROS_WARN("[A*]{sucess}  Time in A*  is %f ms, path length is %f m", (time_2 - time_1).toSec() * 1000.0, currentPtr -> gScore * resolution);            
      return;
    }
...
vector<Vector3d> AstarPathFinder::pathSimplify(const vector<Vector3d> &path,
  ...
  for(int i = 1; i < end - 1; i++){
    double d = distancePointLine(path[0], path[end-1], path[i]);
    if(d > dmax){
      index = i;
      dmax = d;
    }
  ...
  if(dmax > path_resolution){
    vector<Vector3d> a(path.begin(), path.begin() + index);
    vector<Vector3d> b(path.begin() + index, path.end());
    vector<Vector3d> sub1 = pathSimplify(a, path_resolution);
    vector<Vector3d> sub2 = pathSimplify(b, path_resolution);
    subPath.insert(subPath.begin(), sub2.begin(), sub2.end());
    subPath.insert(subPath.begin(), sub1.begin(), sub1.end()-1);
  }else{
    subPath.push_back(path[0]);
    subPath.push_back(path[end-1]);
  }

  return subPath;
}
double AstarPathFinder::distancePointLine(const Vector3d& start, const Vector3d& end, const Vector3d& cur){
  Vector3d v1 = start - end;
  Vector3d v2 = start - cur;
  double cos = v1.dot(v2) / (v1.norm() * v2.norm());
//   double angle = acos(cos) * 180 /M_PI;
  double angle = acos(cos);
  return v2.norm() * sin(angle);
}
...
int AstarPathFinder::safeCheck(MatrixXd polyCoeff, VectorXd time) {
   ...
   *
   * **/
  // segment return start from 1 
  int m = time.size();
  for(int i = 0; i < m; i++){
    double t_delta = time[i] / 10;
    for(double t = t_delta; t <= time[i]; t += t_delta){
      Vector3d coord = getPosPoly(polyCoeff, i, t);
      Vector3i index = coord2gridIndex(coord);
      if (index(0) < 0 || index(0) >= GLX_SIZE ||
          index(1) < 0 || index(1) >= GLY_SIZE ||
          index(2) < 0 || index(2) >= GLZ_SIZE) {
        unsafe_segment = i + 1;
