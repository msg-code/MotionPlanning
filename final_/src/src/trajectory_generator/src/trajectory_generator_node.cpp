...
// 使用最优化理论
bool is_based_optimization = false;
// for replanning
enum STATE {
  INIT,
  WAIT_TARGET,
...
void visSimplyPath(MatrixXd nodes);
...
bool trajGeneration() {
  ...
  cout << "orgin path size = " << grid_path.size() << endl;

  MatrixXd orgin_path(int(grid_path.size()), 3);
  for (int k = 0; k < int(grid_path.size()); k++) {
    orgin_path.row(k) = grid_path[k];
  }
  visPath(orgin_path);
   ...
   *
   * **/
  cout << "getpath sucess!" << endl;
  grid_path = _astar_path_finder->pathSimplify(grid_path, _path_resolution);
  cout << "pathsimplify path length is " <<  grid_path.size()<< endl;
...
void trajOptimization(Eigen::MatrixXd path) {
  ...
  cout << "time allocation sucess!" << endl;
  ...
  if(is_based_optimization){
    _polyCoeff =
              _trajGene->minimumJerkTrajGen(path, vel, acc, _polyTime);
  }else{
    _polyCoeff =
              _trajGene->PolyQPGeneration(_dev_order, path, vel, acc, _polyTime);
  }
  cout << "PolyQpgeneration sucess!" << endl;
  ...
  while (unsafe_segment != -1) {
    ...
    repath.row(unsafe_segment) = (path.row(unsafe_segment) + path.row(unsafe_segment - 1)) / 2;
    for(int i = unsafe_segment + 1; i < path.rows() +1 ; i++){
      repath.row(i) = path.row(i - 1);
    }
    // for(int i = 0; i< repath.rows(); i++){
    //   cout << repath.row(i)<< " ";
    // }
    // cout << "repath size " << repath.rows()<<endl;
    // visPath(repath);
    path = repath;
    _polyTime = timeAllocation(repath);
    if(is_based_optimization){
      _polyCoeff =
                _trajGene->minimumJerkTrajGen(repath, vel, acc, _polyTime);
    }else{
      _polyCoeff =
  ...
  if(count != 0){
    cout << "reoptimize count:" << count << endl; 
    visSimplyPath(repath);
  }else{
    visSimplyPath(path);
  }
  visTrajectory(_polyCoeff, _polyTime);
...
VectorXd timeAllocation(MatrixXd Path) {
  ...
  for(int i = 1; i < Path.rows(); i++){
    dist = (Path.row(i) - Path.row(i-1)).norm();
    const double t = _Vel / _Acc;
    const double d = 0.5 * _Acc * t * t;

    if (dist < d + d)
    {
      time[i-1] = 2.0 * sqrt(dist / _Acc);
    }
    else
    {
      time[i-1] = 2.0 * t + (dist - 2.0 * d) / _Vel;
    }
  }
  return time;
}
...
int main(int argc, char **argv) {
  ...
  nh.param("planning/is_based_optimization", is_based_optimization, true);
